...
class GatedMaskedConvR(nn.Module):
    ...
    def __init__(self, c_in, **kwargs):
        ...
        """
        super().__init__()
        self.conv_vert = VerticalStackConvolutionR(c_in, c_out=2*c_in, mask_center=False, **kwargs)
        self.conv_horiz = HorizontalStackConvolutionR(c_in, c_out=2*c_in, mask_center=False, **kwargs)
        self.conv_vert_to_horiz = nn.Conv2d(2*c_in, 2*c_in, kernel_size=1, padding=0)
        self.conv_horiz_1x1 = HorizontalStackConvolutionR(c_in, c_in, kernel_size=1, mask_center=False)
...
class DIYPixelCNN(nn.Module):
    def __init__(self, input_channels, hidden_channels=66, kernel_size = 3):
        ...
        self.conv_hor_init = HorizontalStackConvolutionR(c_in=input_channels, c_out=hidden_channels, mask_center=True, kernel_size=kernel_size)
        self.conv_ver_init = VerticalStackConvolutionR(c_in=input_channels, c_out=hidden_channels, mask_center=False, kernel_size=kernel_size)
        ...
        self.masked = nn.ModuleList([
            ...
            GatedMaskedConvR(c_in=hidden_channels, kernel_size=kernel_size),
            GatedMaskedConvR(c_in=hidden_channels, kernel_size=kernel_size, dilation=4),
            GatedMaskedConvR(c_in=hidden_channels, kernel_size=kernel_size),
            GatedMaskedConvR(c_in=hidden_channels, kernel_size=kernel_size, dilation=2),
            GatedMaskedConvR(c_in=hidden_channels, kernel_size=kernel_size),
            GatedMaskedConvR(c_in=hidden_channels, kernel_size=kernel_size, dilation=4),
